    ...
    public partial class App : Application
    {
        ...
        void App_DispatcherUnhandledException(object sender, System.Windows.Threading.DispatcherUnhandledExceptionEventArgs e)
        {
            try
            {
                ...
                MessageBox.Show("UI线程异常:" + e.Exception.Message,"出错了",MessageBoxButton.OK,MessageBoxImage.Error);
            }
            catch (Exception ex)
            {
                log.LogW.NewStopLog("在UI线程上引发无法处理的异常" + e.Exception.Message);
                log.LogW.NewStopLog("即将退出!");
                //此时程序出现严重异常，将强制结束退出
                MessageBox.Show("UI线程发生致命错误！","出错了", MessageBoxButton.OK, MessageBoxImage.Error);
            }
